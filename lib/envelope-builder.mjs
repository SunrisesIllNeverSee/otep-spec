...
export function buildEnvelope(telemetry, options = {}) {
  ...
  const provenanceLevel = options.provenance_level ?? "self-reported";

  // Validate privacy_mode against the shared canonical set. The builder's
  // contract guarantees a schema-conforming envelope; an unknown privacy mode
  // would produce an envelope that fails the protocol schema.
  if (!PRIVACY_MODES.includes(privacyMode)) {
    throw new Error(
      `buildEnvelope: privacy_mode '${privacyMode}' is not a valid TTEOP privacy mode — ` +
        `must be one of: ${PRIVACY_MODES.join(", ")}`,
    );
  }

  // Validate provenance_level against the shared canonical set. Same rationale:
  // an unknown provenance level produces a non-conforming envelope.
  if (!PROVENANCE_LEVELS.includes(provenanceLevel)) {
    throw new Error(
      `buildEnvelope: provenance_level '${provenanceLevel}' is not a valid TTEOP provenance level — ` +
        `must be one of: ${PROVENANCE_LEVELS.join(", ")}`,
    );
  }

  // SRP-SIG-001: a "signed" provenance level requires a signature object in
  // extensions (algorithm, key_fingerprint, value, signed_fields). This builder
  // does not accept signature input, so it cannot produce a conforming signed
  // envelope. Reject "signed" at construction time rather than silently emitting
  // a non-conforming envelope with signature_status "unsigned".
  if (provenanceLevel === "signed") {
    throw new Error(
      "buildEnvelope: provenance_level 'signed' is not supported by this builder — " +
        "a signed envelope requires a signature object (algorithm, key_fingerprint, value, signed_fields) " +
        "per SRP-SIG-001, but no signature input parameter is accepted",
    );
  }
