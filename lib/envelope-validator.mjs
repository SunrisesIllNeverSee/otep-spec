 ...
 *
 * Resolution rules (SPEC §26.4/26.5, CodeRabbit minor #2):
 *   - Alias-only (legacy present, canonical absent): normalize alias → canonical.
 *   - Matching pair (both present, same value): keep canonical, drop alias.
 *   - Conflicting pair (both present, different values): REJECT. The alias is
 *     dropped and the canonical value is retained, but a conflict error is
 *     returned so the caller can fail validation. Silently choosing one
 *     conflicting value would create cross-implementation ambiguity.
 *
 * Does not mutate the input. Returns { normalized, conflicts } where
 * `conflicts` is an array of human-readable error strings (empty when ok).
 */
export function normalizeLegacyMetrics(envelope) {
  if (!envelope || typeof envelope !== "object" || !envelope.metrics || typeof envelope.metrics !== "object") {
    return { normalized: envelope, conflicts: [] };
  }
  const metrics = envelope.metrics;
  const hasLegacy = Object.keys(LEGACY_METRIC_ALIASES).some(k => k in metrics);
  if (!hasLegacy) return { normalized: envelope, conflicts: [] };

  const normalized = { ...metrics };
  const conflicts = [];
  for (const [legacy, canonical] of Object.entries(LEGACY_METRIC_ALIASES)) {
    if (!(legacy in normalized)) continue;
    if (canonical in normalized) {
      // Both present: accept only if they agree; reject on conflict.
      if (!Object.is(normalized[legacy], normalized[canonical])) {
        conflicts.push(
          `metrics.${legacy} (${JSON.stringify(normalized[legacy])}) conflicts with canonical metrics.${canonical} (${JSON.stringify(normalized[canonical])}) — alias and canonical must agree (SPEC §26.4/26.5)`
        );
      }
      // Matching pair or conflict: drop the alias, keep canonical.
    } else {
      // Alias-only: normalize to canonical.
      normalized[canonical] = normalized[legacy];
    }
    delete normalized[legacy];
  }
  return { normalized: { ...envelope, metrics: normalized }, conflicts };
}
...
export function validateEnvelopeSchema(envelope, schema) {
  ...
  const { normalized, conflicts } = normalizeLegacyMetrics(envelope);
...
export function validateEnvelope(envelope, options = {}) {
  ...
  // Conflicting alias/canonical pairs are hard rejects (CodeRabbit minor #2).
  const { normalized, conflicts } = normalizeLegacyMetrics(envelope);

  // Schema validation (skipped on conflict — the envelope is ambiguous)
  let schemaValid;
  let schemaErrors;
  if (conflicts.length > 0) {
    schemaValid = false;
    schemaErrors = conflicts;
  } else {
    const result = validateEnvelopeSchema(normalized);
