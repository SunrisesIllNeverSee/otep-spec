 ...
 * If the canonical name already exists, the legacy alias is dropped (canonical wins).
 */
export function normalizeLegacyMetrics(envelope) {
  ...
  for (const [legacy, canonical] of Object.entries(LEGACY_METRIC_ALIASES)) {
    if (legacy in normalized && !(canonical in normalized)) {
...
export function validateEnvelopeSchema(envelope, schema) {
  ...
  const normalized = normalizeLegacyMetrics(envelope);
